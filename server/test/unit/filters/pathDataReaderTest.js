...
module.exports = testCase({
    ...
    'should enrich data on path param at request': function (assert) {
      request = {method:"GET",url:"/services/1/carlos",data:{client:{ address:'qnd 19'}}};
      ...
      assert.equal(request.data.client.id,1);
      assert.equal(request.data.client.name,'carlos');
      assert.equal(request.data.client.address, 'qnd 19');
      assert.ok(invoked);
      assert.done();
    },
